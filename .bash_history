...
echo "Working in: $(pwd)"
# Init repo if needed
if [ ! -d .git ]; then   echo "-> Initializing git repo";   git init; fi
# Stage & commit (no error if nothing to commit)
git add .
git commit -m "Deploy: update project" 2>/dev/null || echo "⚑ no changes to commit"
# Ensure main branch name
git branch -M main 2>/dev/null || true
# If origin exists show it, otherwise try to create it with gh
if git remote get-url origin >/dev/null 2>&1; then   echo "-> Remote 'origin' already set to: $(git remote get-url origin)"; else   if command -v gh >/dev/null 2>&1; then     REPO_NAME="${1:-web11-deploy}";     echo "-> Creating GitHub repo '${REPO_NAME}' and adding remote origin (using gh)"
    gh repo create "$REPO_NAME" --public --source=. --remote=origin --push || {       echo "gh create failed (maybe repo exists). Please run the next line manually if needed:";       echo "  git remote add origin https://github.com/$(gh api user --jq .login)/$REPO_NAME.git";     };   else     echo "⚠️ 'gh' CLI not found and no remote 'origin' configured.";     echo "Create a repo on GitHub and run:";     echo "  git remote add origin https://github.com/YOUR_USERNAME/YOUR_REPO.git";     echo "Then run: git push -u origin main";     exit 1;   fi; fi
# Finally push
